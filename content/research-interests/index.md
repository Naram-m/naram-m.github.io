...
- **Markov decision processes (MDPs) and control** incorporate lasting consequences: actions affect immediate rewards and future states, such as battery reserves. Learning in unknown MDPs requires reinforcement learning; see [Chapter 38 of Lattimore and Szepesvári][bandits]. Control theory captures the same interplay through system dynamics, and [Hazan and Singh][control] study it from an online learning perspective as non-stochastic control.
...
Finally, I study systems with memory through <dfn>non-stochastic control</dfn>. As in an MDP, actions affect future states, but the dynamics and convex costs may be adversarial, and we compete with the best **policy** in a specified class, chosen in hindsight. My thesis shows how forecasts over an extended horizon help account for the lasting effects of today's actions.
