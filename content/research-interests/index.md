...
The central performance measure is *regret*: the algorithm's accumulated cost relative to a specified benchmark, such as the best fixed decision chosen with hindsight. For content delivery networks, this could be the fixed collection of files that would have served users best. A suitable algorithm makes its average excess cost vanish over time, despite never seeing future requests. This possibility is what draws me to the field.

Several mathematical frameworks can represent this problem:
...
- **Markov decision processes (MDPs)** incorporate lasting consequences of an action: an action affects immediate rewards and future states, such as queue lengths or battery reserves. Learning in unknown MDPs connects exploration with long-term planning in reinforcement learning; see [Chapter 38 of Lattimore and Szepesvári][bandits].
...
My [doctoral thesis][thesis], asks how algorithms can use **untrusted predictions while maintaining worst-case regret guarantees**. The objective combines robustness to inaccurate forecasts with gains when forecasts are informative. Optimism means adapting to prediction quality, which need not be known in advance. 
